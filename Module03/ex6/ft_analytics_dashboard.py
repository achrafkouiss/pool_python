...
unique_achievements = {
    achievement for achievement in data_dictionary["achievements"]
    }
...
count_pls = len({p for p in players})
total_unique_achievements = sum(
    [players[p]["achievements_count"] for p in players]
    )
average_score = sum(players_score[key] for key in players_score) / count_pls
top_performer = max(players_score, key=get_dict_value)

print(f"Total players: {count_pls}")
print(f"Total unique achievements: {total_unique_achievements}")
print(f"Average score: {average_score:.2f}")
print(
    f"{top_performer} ({players_score[top_performer]} points, "
